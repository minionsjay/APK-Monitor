...
def capture_pcap(ip, port, duration=30):
    ...
    # 在真机上执行 tcpdump
    cmd = f"adb -s 192.168.1.16:41879 shell \"su 0 -c 'timeout {duration} tcpdump -i any host {ip} -c 200 -w /sdcard/forensics.pcap'\""
    try:
        subprocess.run(cmd, shell=True, timeout=duration+10, capture_output=True)
        # 拉取 pcap
        subprocess.run(f"adb -s 192.168.1.16:41879 pull /sdcard/forensics.pcap {pcap_file}",

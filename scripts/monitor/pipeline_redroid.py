#!/usr/bin/env python3
"""Redroid流水线 — 用Windows adb.exe通过SSH隧道连接Redroid容器
- 安装APK到Redroid(127.0.0.1:5555)
- 读取sdk_forwarder_fixed.json(用cat,不需要root)
- 16秒/APK,比云手机快15倍
"""
import subprocess, json, os, time, re, csv, zipfile, base64, threading, queue
...
def get_launch_activity(pkg, apk_path):
    """从APK获取启动Activity"""
    r = subprocess.run([AAPT, 'dump', 'badging', apk_path], capture_output=True, text=True, timeout=10)
    for line in r.stdout.split('\n'):
    ...
    for line in r.stdout.split('\n'):
        if 'LaunchActivity' in line:
            parts = line.strip().split('/')
            if len(parts) >= 2: return parts[1].split()[0]
    return None

def get_proxy_nodes(pkg, max_wait=30):
    """读取sdk_forwarder_fixed.json(每5秒检查)"""
...
def install_worker():
    ...
    while True:
        ...
        t3 = time.time()
        try:
            r = subprocess.run([ADB, '-s', ADB_DEVICE, 'install', '-r', win_apk_path], capture_output=True, text=True, timeout=120)
        ...
        t4 = time.time()
        # 获取启动Activity
        launch_act = get_launch_activity(pkg, apk_path)
        if launch_act:
            run_adb(['shell', f'am start -n {pkg}/{launch_act}'], timeout=10)
        else:
...
def generate_html(db, all_proxy):
    ...
    parts = []
    parts.append('<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>Redroid APK检测报告</title>')
    parts.append('<style>body{font-family:sans-serif;margin:20px;background:#f5f5f5}h1{color:#333;border-bottom:3px solid #e74c3c;padding-bottom:10px}h2{color:#2c3e50;border-left:4px solid #3498db;padding-left:10px;margin-top:30px}table{border-collapse:collapse;width:100%;margin:10px 0;background:white;box-shadow:0 1px 3px rgba(0,0,0,.1)}th{background:#2c3e50;color:white;padding:10px;text-align:left}td{padding:8px;border-bottom:1px solid #ddd}tr:hover{background:#f0f0f0}.tag{display:inline-block;padding:3px 10px;border-radius:3px;font-size:13px;margin:1px}.tag-aliyun{background:#ff6600;color:white}.tag-tencent{background:#00a4ef;color:white}.tag-huawei{background:#e60012;color:white}.tag-unknown{background:#95a5a6;color:white}.stat-card{background:white;padding:15px;border-radius:8px;box-shadow:0 2px 5px rgba(0,0,0,.1);display:inline-block;margin:5px;text-align:center;min-width:120px}.stat-number{font-size:2em;font-weight:bold;color:#2c3e50}img.icon{width:100px;height:100px;border-radius:10px}img.screenshot-thumb{width:120px;border-radius:5px;border:1px solid #ddd;cursor:pointer}.mono{font-family:monospace;font-size:13px}.lightbox{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.9);z-index:9999;justify-content:center;align-items:center}.lightbox.active{display:flex}.lightbox img{max-width:90%;max-height:85%}.lb-close{position:absolute;top:20px;right:30px;color:white;font-size:40px;cursor:pointer}</style></head><body>')
    ...
    parts.append('<h1>Redroid APK检测报告</h1>')
    parts.append('<p>生成时间: ' + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + ' | Redroid(ARM64,Android10) | 腾讯云</p>')
    ...
    html_path = f'{RESULTS_DIR}/report_redroid.html'
...
def generate_report(db, all_proxy):
    ...
    save_json(f'{RESULTS_DIR}/run_result_redroid.json', run_result)
    old_hw_path = '/tmp/old_hw_ips_redroid.json'
    ...
    try:
        ...
        subprocess.run(['git', 'commit', '-m', f'Redroid: {stats["installed"]}APK, {len(all_proxy)}节点'], cwd=BASE_DIR, capture_output=True, timeout=30)
...
def main():
    ...
    state = load_state()
    print(f"=== Redroid流水线 (ARM64, Android10, 127.0.0.1:5555) ===")
